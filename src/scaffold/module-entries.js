...
function prepareModuleEntryPlan(context) {
  const active = getActiveDescriptors(context);
...
function prepareWrite(context, descriptor) {
  ...
  return makeRecord('refresh', replaceMarkedBlock(existing, eolBlock, MODULE_BEGIN, MODULE_END), descriptor, targetPath, eolBlock);
}
...
function prepareRetirement(context, record) {
  ...
  if (record.fileDigest && record.fileDigest === observedDigest) {
    return { action: 'remove', kind: 'retire-module-entry', observedDigest, targetPath };
  }
  return { action: 'retire', content: removeMarkedBlock(existing, MODULE_BEGIN, MODULE_END), kind: 'retire-module-entry', observedDigest, targetPath };
}

function makeRecord(action, content, descriptor, targetPath, block) {
  return {
    action,
