...
function writeStatusFile(context) {
  const { commands, options, targetDir, workDirectory, printAction } = context;
  const statusPath = safeResolveInside(targetDir, STATUS_FILE, 'status target');
  const statusContent = `${JSON.stringify(createStatus({ ...options, commands }, { workDirectory }), null, 2)}\n`;
