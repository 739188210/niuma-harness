...
description: "Jereh GraphQL Spec-First 流程：编写 {name}.graphqls + queries，用 DGS codegen 生成 Java + TypeScript 客户端，实现 Spring GraphQL DataFetcher，在 Vue 中消费。触发词：GraphQL schema, DataFetcher, GraphQL query, 联查, GraphQL客户端."
metadata:
  author: jereh
  version: "2.0.0"
---
...
│   └── queries/                      # 操作文档（query / mutation），客户端代码生成输入
│       ├── {name}-by-id.graphql
│       ├── {name}-page.graphql
│       └── ...
│
...
- **查询文档 `*.graphql` 放 `{name}-spec/queries/`**。TS 客户端基于这些文档生成 typed 操作。
- **schema 命名**：类型 PascalCase，字段 camelCase，枚举 UPPER_SNAKE。
