...
test('first init preserves pre-existing experience guide without mutation', () => {
  const workspace = tempDir();
  const guidePath = path.join(workspace, 'ai-harness', 'docs', 'experience', 'README.md');
  fs.mkdirSync(path.dirname(guidePath), { recursive: true });
  fs.writeFileSync(guidePath, 'project experience guide\n', 'utf8');
  const before = snapshotTree(workspace);
  const result = run(['init', workspace, '--agent', 'claude', '--harness-dir', 'ai-harness']);

  assert.notStrictEqual(result.status, 0, result.stdout);
  assert.match(result.stderr, /ai-harness\/docs\/experience\/README\.md/);
  assertNoPath(path.join(workspace, 'CLAUDE.md'));
  assertNoPath(path.join(workspace, 'ai-harness', 'manifest.json'));
  assertTreeUnchanged(workspace, before);
});

test('dry-run preserves pre-existing experience guide without mutation', () => {
  const result = assertFreshFailureLeavesNoChanges('claude', (workspace) => {
    const guidePath = path.join(workspace, 'ai-harness', 'docs', 'experience', 'README.md');
    fs.mkdirSync(path.dirname(guidePath), { recursive: true });
    fs.writeFileSync(guidePath, 'project experience guide\n', 'utf8');
