...
test('explicit modules initialize root routing and local supplements', () => {
  ...
  const rootClaudeEntry = read(path.join(workspace, 'CLAUDE.md'));
  assert.match(rootClaudeEntry, /read the root \[`AGENTS\.md`\]\(AGENTS\.md\)/);
  assert.doesNotMatch(rootClaudeEntry, /module-topology/);
  const rootAgentsEntry = read(path.join(workspace, 'AGENTS.md'));
  assert.match(rootAgentsEntry, /module-topology/);
  assert.match(rootAgentsEntry, /read their local supplements/);
  assert.match(rootAgentsEntry, /Their single source of truth is[\s\S]*harness\/docs\/project-context\.md/);
  assert.match(rootAgentsEntry, /module-local durable facts belong in the applicable[\s\S]*module entry/i);
