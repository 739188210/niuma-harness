...
const { initWorkspace } = require('./cli-fixtures');
...
function assertCommonHarnessShape(workspace, options = {}) {
  ...
  assertNoPath(path.join(harnessRoot, 'docs', 'policy', 'secret-leak.md'));
  assertNoPath(path.join(harnessRoot, 'docs', 'policy', 'untrusted-content.md'));
  ...
  assertNoPath(path.join(harnessRoot, 'docs', 'process'));
