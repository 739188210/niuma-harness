...
- Route: for non-trivial work, route through `{{HARNESS_DIR}}/docs/process/task-triage.md`; it selects only the Harness navigation, project knowledge, Policy, module, decision, experience, recovery, and task-material documents that apply before choosing a playbook. Then open only the selected process for workflow-specific materials and gates. Skip routing only for trivial single-step tasks. (depth: `{{HARNESS_DIR}}/docs/process/`)
