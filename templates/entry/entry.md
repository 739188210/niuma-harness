...
The loop above is all that stays in context. Each phase names the one file to open when it needs detail. Coding preferences live in `{{HARNESS_DIR}}/docs/rules/`; load only the relevant rule files on demand. Full loop spec: `{{HARNESS_DIR}}/docs/layers/07-loop.md`.
