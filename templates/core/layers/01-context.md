...
2. For non-trivial work, use `{{HARNESS_DIR}}/docs/process/task-triage.md` to select conditional reading and a playbook. Triage decides whether `{{HARNESS_DIR}}/docs/index.md`, `{{HARNESS_DIR}}/docs/project-context.md`, or other Harness material is relevant; do not load them merely because the task starts.
...
5. Apply only the relevant triage conditions for Policy, module routing, ADRs, experience, fact-scope refresh, or task-local recovery material. For a selected fact scope, use its `Refresh when` condition and listed current sources to refresh only the task-relevant entry; current workspace evidence takes precedence over retained context. Do not enumerate decision, experience, or task-work directories speculatively; read a record only when the request, current workspace evidence, or an explicit reference establishes that it may apply.
