...
1. First inspect request-named files and the smallest relevant current source, configuration, build, test, README, or command evidence. Classify each task-relevant source as current verifiable fact, current navigation/runtime material, governance or reusable knowledge, or historical/task material. A file existing in the repository is not automatically a current fact.
2. When task-material profile selection, conditional Harness reading, or task-material selection is needed, use `{{HARNESS_DIR}}/docs/layers/03-process.md`. Process decides whether `{{HARNESS_DIR}}/docs/index.md`, `{{HARNESS_DIR}}/docs/project-context.md`, or other Harness material is relevant; do not load them merely because the task starts.
3. When Process selects stable project facts, read only the matching fact scope, task-relevant headings, current README files, and verified runbooks needed to locate operating guidance. `Task fact routing`, when present, is an optional locator for existing stable-fact headings, not an authority or a second code map; use it only when it matches the task, then verify against current workspace evidence.
4. Inspect current source, configuration, build definitions, tests, and command output when a fact affects the task; do not rely only on stale notes, plans, migration material, or experience. Current verifiable evidence determines task-specific facts. Do not scan the whole project merely because a relevant fact scope is missing; inspect the smallest workspace evidence that can answer the task.
5. Apply only the relevant Process conditions for Policy, module routing, experience, fact-scope refresh, or task-local recovery material. For a selected fact scope, use its `Refresh when` condition and listed current sources to refresh only the task-relevant entry; current workspace evidence takes precedence over retained context. Do not enumerate experience or task-work directories speculatively; read a record only when the request, current workspace evidence, or an explicit reference establishes that it may apply.
...
7. Use Rules and active experience as reusable guidance. Use historical materials only as background, search terms, or hypotheses until verified against higher-priority current sources.
