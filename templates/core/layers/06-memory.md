...
3. When updating `{{HARNESS_DIR}}/docs/project-context.md`, record only verified durable facts, and only when the task verifies them and they have durable value. Give each fact scope a source, scope, and freshness boundary when useful; current workspace evidence always overrides the index.
